...
int bl_iw_priv_handler(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
    ...
    buf = kzalloc(wrqu->data.length + 1, GFP_KERNEL);
    if (buf == NULL)
        return -ENOMEM;

    if (wrqu->data.length > 0) {
        if (copy_from_user(buf, wrqu->data.pointer, wrqu->data.length)) {
            kfree(buf);
            return -ENOMEM;
        }
    }
    buf[wrqu->data.length] = 0;

    switch (info->cmd) {
        case BL_IOCTL_VERSION:
        {
            u8 ret_buf[128];
            wrqu->data.length += sprintf(ret_buf, "\nBL Wlan Driver Version is %s\n", RELEASE_VERSION);
            if(extra) {
                memcpy(extra, ret_buf, wrqu->data.length);
            } else {
                if (copy_to_user(wrqu->data.pointer, ret_buf, wrqu->data.length))
                    ret = -1;
            }
        }
            break;
...
int bl_ioctl(struct net_device *dev, struct ifreq *ifreq, int cmd)
{
    ...
    return bl_iw_priv_handler(dev, &info, &wrq->u, NULL);
}
...
static const iw_handler bl_iw_priv_hdl_array[] = {
    [BL_IOCTL_VERSION - SIOCIWFIRSTPRIV] = (iw_handler)bl_iw_priv_handler,
};
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 6, 0)
     ndev = bl_interface_add(bl_hw, "wlan%d", NL80211_IFTYPE_STATION, NULL);
#else
