...
//			bl_txq_stop(txhdr->sw_hdr->txq, BL_TXQ_STOP_FULL);
		...
		if(txhdr->sw_hdr->desc.host.flags & TXU_CNTRL_MGMT){
				txq_num[mpa_tx_data.pkt_cnt] = ipc_host_txdesc_push(bl_hw->ipc_env, txhdr->sw_hdr->hw_queue, 0, skb);
...
*/
    ...
    if(ret) {
		...
		if (cnt >= BL_SDIO_RD_WR_RETRY) {
			printk("retry over 10 times, free[%d] pkt of queue[%d]\n", mpa_tx_data.pkt_cnt, txq_num[mpa_tx_data.pkt_cnt]);
#ifdef BL_FW_RETRY
			if(txhdr->sw_hdr->desc.host.flags & TXU_CNTRL_MGMT)
#endif
			{
				do {
					skb = skb_dequeue_tail(&bl_hw->transmitted_list[txq_num[mpa_tx_data.pkt_cnt]]);
					txhdr = (struct bl_txhdr *)skb->data;
					kmem_cache_free(bl_hw->sw_txhdr_cache, txhdr->sw_hdr);
					consume_skb(skb);
				} while(--mpa_tx_data.pkt_cnt);
			}
		}
	}
